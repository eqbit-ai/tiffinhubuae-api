...
router.post('/auth/request-otp', async (req, res) => {
    try {
        ...
        await prisma_1.prisma.customerOTP.create({
            data: {
                phone_number,
                otp_code: otpCode,
                merchant_id,
...
router.post('/auth/verify-otp', async (req, res) => {
    try {
        ...
        if (otpRecord.otp_code !== otp) {
            await prisma_1.prisma.customerOTP.update({
...
router.get('/me', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
...
router.put('/me', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
...
router.get('/payments', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
...
router.post('/renew', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
        const merchant = await prisma_1.prisma.user.findUnique({ where: { id: customer.merchant_id } });
        if (!merchant) {
...
router.post('/pause', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
        ...
        const startDate = (0, date_fns_1.parseISO)(pause_start);
        const endDate = (0, date_fns_1.parseISO)(pause_end);
        ...
        const pauseDays = (0, date_fns_1.differenceInDays)(endDate, startDate);
        if (pauseDays > 30) {
        ...
        const newEndDate = currentEndDate ? (0, date_fns_1.addDays)(currentEndDate, pauseDays) : null;
        // Build pause history
        ...
        await prisma_1.prisma.customer.update({
            ...
            data: {
                ...
                original_end_date: customer.end_date ? (0, date_fns_1.format)(customer.end_date, 'yyyy-MM-dd') : null,
        ...
        res.json({
            ...
            pause_days: pauseDays,
            new_end_date: newEndDate ? (0, date_fns_1.format)(newEndDate, 'yyyy-MM-dd') : null,
        });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post('/resume', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
...
router.get('/orders', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
        const orders = await prisma_1.prisma.oneTimeOrder.findMany({
            where: { customer_id: customer.id },
            orderBy: { created_at: 'desc' },
            take: 50,
        });
...
router.get('/skips', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
...
router.post('/skips', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
        ...
        const existing = await prisma_1.prisma.tiffinSkip.findFirst({
            where: {
                customer_id: customer.id,
                skip_date,
        ...
        const skip = await prisma_1.prisma.tiffinSkip.create({
            data: {
                customer_id: customer.id,
                customer_name: customer.full_name,
                skip_date,
...
router.delete('/skips/:skipId', auth_1.customerAuthMiddleware, async (req, res) => {
    try {
        const customer = req.customer;
        const skip = await prisma_1.prisma.tiffinSkip.findFirst({
            where: { id: req.params.skipId, customer_id: customer.id },
        });
        if (!skip) {
...
router.get('/join/:merchantId', async (req, res) => {
    try {
        const merchant = await prisma_1.prisma.user.findUnique({ where: { id: req.params.merchantId } });
        if (!merchant)
            return res.status(404).json({ error: 'Merchant not found' });
        res.json({
            business_name: merchant.business_name || 'Tiffin Service',
...
router.post('/join/:merchantId', async (req, res) => {
    try {
        const merchant = await prisma_1.prisma.user.findUnique({ where: { id: req.params.merchantId } });
        if (!merchant)
            return res.status(404).json({ error: 'Merchant not found' });
        ...
        const customer = await prisma_1.prisma.customer.create({
            data: {
                ...
                active: false,
                is_trial: isTrial,
                trial_end_date: isTrial ? (0, date_fns_1.addDays)(new Date(), 3) : null,
...
<p>Please log in to your dashboard to approve or reject this registration.</p>`,
            ...
            console.error('[Portal] Send failed:', e.message);
        }
        // Create notification
        await prisma_1.prisma.notification.create({
            data: {
...
router.get('/:token', async (req, res) => {
    try {
        const customer = await prisma_1.prisma.customer.findFirst({
            where: { portal_token: req.params.token, is_deleted: false },
        });
        if (!customer)
            return res.status(404).json({ error: 'Not found' });
        const owner = await prisma_1.prisma.user.findUnique({ where: { id: customer.created_by } });
        ...
        res.json({
            ...
            business: {
                name: owner?.business_name || 'Tiffin Service',
            },
            menu: menuItems.map(m => ({
                name: m.name,
                description: m.description,
...
router.post('/:token/skip', async (req, res) => {
    try {
        ...
        const { skip_date, meal_type } = req.body;
        if (!skip_date)
            return res.status(400).json({ error: 'skip_date required' });
        // Check if skip already exists
        const existing = await prisma_1.prisma.tiffinSkip.findFirst({
            where: {
                customer_id: customer.id,
...
router.delete('/:token/skip/:skipId', async (req, res) => {
    try {
        const customer = await prisma_1.prisma.customer.findFirst({
            where: { portal_token: req.params.token, is_deleted: false },
        });
